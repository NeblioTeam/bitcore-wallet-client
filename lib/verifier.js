...
Verifier.checkAddress = function(credentials, address) {
  log.debug('checkAddress before check');
  $.checkState(credentials.isComplete());

  log.debug('addr type: '+address.type+' or '+credentials.addressType+' pubkeyring: '+credentials.publicKeyRing+' path: '+address.path+' m: '+credentials.m+' net: '+credentials.network);
  ...
  log.debug('local address: '+local.address);
  log.debug('address: '+address.address);
  log.debug('local.publicKeys: '+local.publicKeys);
  log.debug('address.publicKeys: '+address.publicKeys)
