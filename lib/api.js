...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  log.debug('Net: '+opts.network+' Pass: '+opts.passphrase+' Lang: '+opts.language+' Acc: '+opts.account);
  $.checkArgument(arguments.length <= 1, 'DEPRECATED: only 1 argument accepted.');
  $.checkArgument(_.isUndefined(opts) || _.isObject(opts), 'DEPRECATED: argument should be an options object.');
 ...
 * @param {Boolean} opts.isChange[=false] - Create change address
 * @param {Callback} cb
 * @returns {Callback} cb - Return error or the address
 */
API.prototype.createAddress = function(opts, cb) {
  log.debug('create address Before checkstate');
  $.checkState(this.credentials && this.credentials.isComplete());
...
/**
 * Returns the latest unused change address or creates a new one.
 *
 * @param {Callback} cb
 * @returns {Callback} cb - Return error or the address
 */
API.prototype.getNextChangeAddress = function(opts, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  ...
  self._doPostRequest('/v3/addresses/getchange', opts, function(err, address) {
    if (err) return cb(err);
