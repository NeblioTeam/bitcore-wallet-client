...
Credentials.createWithMnemonic = function(network, passphrase, language, account, opts) {
  ...
  console.log('PK: '+x.xPrivKey);
  x._expand();
  x.mnemonic = m.phrase;
  console.log('M3: '+x.mnemonic);
...
Credentials.prototype._expand = function() {
  ...
  console.log('_expand '+'after check');

  var network = Credentials._getNetworkFromExtendedKey(this.xPrivKey || this.xPubKey);
  if (this.network) {
    $.checkState(this.network == network);
    console.log('_expand '+'checkstate net'+network+' '+this.network);
  } else {
    this.network = network;
    console.log('_expand '+'check net '+this.network+' '+network);
  }

  if (this.xPrivKey) {
  	console.log('_expand '+' xPrivKey enter');
    var xPrivKey = new Bitcore.HDPrivateKey.fromString(this.xPrivKey);
    console.log('_expand '+' key: '+xPrivKey);
